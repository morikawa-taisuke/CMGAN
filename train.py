...
parser.add_argument("--epochs", type=int, default=120, help="number of epochs of train")
parser.add_argument("--batch_size", type=int, default=4)
parser.add_argument("--log_interval", type=int, default=500)
parser.add_argument("--decay_epoch", type=int, default=30, help="epoch from which to start lr decay")
parser.add_argument("--init_lr", type=float, default=5e-4, help="initial learning rate")
parser.add_argument("--cut_len", type=int, default=16000*2, help="cut length, default is 2 seconds in denoise and dereverberation")
parser.add_argument("--data_dir", type=str, default='C:\\Users\\kataoka-lab\\Desktop\\sound_data\\mix_data\\subset_DEMAND_hoth_0000dB_0.5sec_1ch', help="dir of VCTK+DEMAND dataset")
...
def ddp_setup(rank, world_size):
    """
    Args:
        rank: Unique identifier of each process # 各プロセスの一意な識別子
        world_size: Total number of processes   # プロセス総数
    """
    os.environ["MASTER_ADDR"] = "localhost"
    os.environ["MASTER_PORT"] = "12355"
    # init_process_group(backend="nccl", rank=rank, world_size=world_size)  # 原文
...
class Trainer:
    ...
    def forward_generator_step(self, clean, noisy):
        ...
        noisy_spec = torch.stft(
            ...
            onesided=True,
            return_complex=True,    # 変更点
        )
        noisy_spec = torch.view_as_real(noisy_spec) # 変更点
        ...
        clean_spec = torch.view_as_real(clean_spec) # 変更点
        ...
        est_spec_uncompress = torch.view_as_complex(est_spec_uncompress)  # 変更点
    ...
    def test(self):
        ...
        gen_loss_total = 0.0
        disc_loss_total = 0.0
        for idx, batch in tenumerate(self.test_ds):
            step = idx + 1
    ...
    def train(self, checkpoint_path=None):
        ...
        if checkpoint_path != None:
            ...
            self.model.load_state_dict(checkpoint["model_state_dict"])  # 学習途中のモデルの読み込み
            self.optimizer.load_state_dict(checkpoint["optimizer_state_G"])  # オプティマイザの読み込み
            self.optimizer_disc.load_state_dict(checkpoint["optimizer_state_G"])  # オプティマイザの読み込み
            # optimizerのstateを現在のdeviceに移す。これをしないと、保存前後でdeviceの不整合が起こる可能性がある。
            for state in self.optimizer.state.values():
                for k, v in state.items():
                    if isinstance(v, torch.Tensor):
                        state[k] = v.to("cuda" if torch.cuda.is_available() else "cpu")
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            loss_D = 0
            loss_G = 0
            for idx, batch in tenumerate(self.train_ds):
                step = idx + 1
            ...
            if self.gpu_id == 0:
                torch.save(self.model.module.state_dict(), path)    # モデルのsave
            scheduler_G.step()  # パラメータの更新  Generator
            scheduler_D.step()  # パラメータの更新  Discriminator
            """ checkpointの作成 """
            torch.save({'epoch': epoch+1,
                        'model_state_dict': self.model.module.state_dict(),
                        'optimizer_state_G': self.optimizer.state_dict(),
                        'optimizer_state_D': self.optimizer_disc.state_dict(),
                        'loss_G': loss_G,
                        'loss_D': loss_D},
                       os.path.join(args.save_model_dir, "CMGAN_checkpoint.pth"))
...
def main(rank: int, world_size: int, args, chcekpoint_path=None):
    ddp_setup(rank, world_size)
    if rank == 0:
        print(args)
        available_gpus = [torch.cuda.get_device_name(i) for i in range(torch.cuda.device_count())]
        print(available_gpus)
    train_ds, test_ds = dataloader.load_data(args.data_dir,
                                             args.batch_size,
                                             2,
                                             args.cut_len)
    trainer = Trainer(train_ds, test_ds, rank)
    trainer.train(checkpoint_path=chcekpoint_path)
    destroy_process_group()


if __name__ == "__main__":
    chcekpoint_path = None
    world_size = torch.cuda.device_count()
    mp.spawn(main, args=(world_size, args, chcekpoint_path), nprocs=world_size)
